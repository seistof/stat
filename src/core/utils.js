...
export function dictionaryNode(ministry = false, territory = false, program = false) {
  ...
  if (territory) {
    ...
    titleName.textContent = 'Справочник: Территории';
    headerName.textContent = 'Название';
    headerCode.textContent = 'Код';
    headerControl.textContent = 'Изменить';
    addNewButton.textContent = 'add';
  ...
  if (program) {
    ...
    headerCode.className = 'ministry-header__code';
    headerControl.className = 'ministry-header__control';
    list.classList.add('ministry-list');
...
export function fillMinistryOrTerritory(data, container, type) {
  if (type === 'ministry') {
    data.forEach((entry, i) => {
      ...
      item.classList.add('item', 'item-solo', `item-${i}`);
      itemName.className = 'item__name';
      itemNameTooltip.className = 'tooltip';
      itemYear.className = 'item__year';
      itemCode.className = 'item__code';
  ...
  if (type === 'program') {
    data.forEach((entry, i) => {
      ...
      const itemControlEdit = document.createElement('span');
      const itemControlDelete = document.createElement('span');
      const itemControlAddEvent = document.createElement('span');

      item.classList.add('item', `item-${i}`, 'item-program');
      ...
      itemControlEdit.classList.add('item__control-edit', 'material-icons', 'button');
      itemControlDelete.classList.add('item__control-delete', 'material-icons', 'button');
      itemControlAddEvent.classList.add('item__control-add-event', 'material-icons', 'button');
      itemControlEdit.textContent = 'create';
      itemControlDelete.textContent = 'delete_forever';
      itemControlAddEvent.textContent = 'add';
      ...
      itemControlEdit.dataset.index = `${i}`;
      itemControlDelete.dataset.index = `${i}`;
      itemControlAddEvent.dataset.index = i;

      item.appendChild(wrapper);
      wrapper.appendChild(itemName);
      itemName.appendChild(itemNameTooltip);
      wrapper.appendChild(itemYear);
      wrapper.appendChild(itemCode);
      wrapper.appendChild(itemControl);
      itemControl.appendChild(itemControlAddEvent);
      itemControl.appendChild(itemControlEdit);
      itemControl.appendChild(itemControlDelete);
      ...
      if (entry.events !== null) {
        ...
        entry.events.forEach((e, k) => {
          ...
          const eventNameTooltip = document.createElement('div');
          ...
          const eventControlEdit = document.createElement('span');
          const eventControlDelete = document.createElement('span');

          eventItem.className = 'event-item';
          eventName.className = 'event-name';
          eventNameTooltip.className = 'tooltip';
          eventYear.className = 'event-year';
          eventCode.className = 'event-code';
          eventControl.className = 'event-control';
          eventControlEdit.classList.add('event__control-edit', 'material-icons', 'button');
          eventControlDelete.classList.add('event__control-delete', 'material-icons', 'button');
          eventControlEdit.textContent = 'create';
          eventControlDelete.textContent = 'delete_forever';
          eventControlEdit.dataset.mainIndex = i;
          eventControlEdit.dataset.index = k;
          eventControlDelete.dataset.mainIndex = i;
          eventControlDelete.dataset.index = k;

          eventName.textContent = e.name;
          eventNameTooltip.textContent = e.name;
          eventYear.textContent = `(${e.from_year} - ${e.to_year})`;
          eventYear.textContent = parseInt(e.from_year) === parseInt(e.to_year) ?
            `(${e.from_year})` : `(${e.from_year} - ${e.to_year})`;
          eventCode.textContent = e.code;

          eventName.appendChild(eventNameTooltip);
          eventItem.appendChild(eventName);
          eventItem.appendChild(eventYear);
          eventItem.appendChild(eventCode);
          eventItem.appendChild(eventControl);
          eventControl.appendChild(eventControlEdit);
          eventControl.appendChild(eventControlDelete);
          eventBox.appendChild(eventItem);
        });
        item.appendChild(eventBox);
      }
      container.appendChild(item);
    });
  }
...
export function addNewWindowMinistryOrTerritory(ministry, territory, program) {
  if (ministry) {
    ...
    shortTitle.textContent = 'Сокращенное:';
  ...
  if (program) {
    ...
    const buttonsCancel = document.createElement('div');
    const eventBox = document.createElement('div');
    addNew.classList.add('add-new-window', 'window-program');
    title.className = 'add-new-title';
    data.className = 'add-new-data';
    nameBox.classList.add('name-box', 'add-new-data-box');
    nameTitle.className = 'name-title';
    ...
    codeValue.className = 'code-value';
    buttonsBox.className = 'buttons-box';
    buttonsAdd.classList.add('buttons-add', 'button');
    buttonsSave.classList.add('buttons-save', 'button');
    buttonsCancel.classList.add('buttons-cancel', 'button', 'close-button');
    ...
    yearTitle.textContent = 'Год:';
    codeTitle.textContent = 'Код:';
    buttonsAdd.textContent = 'Добавить мероприятие';
    buttonsSave.textContent = 'Сохранить';
    buttonsCancel.textContent = 'Отмена';
    yearDiv.textContent = '-';
    shortValue.type = 'text';
    ...
    codeBox.appendChild(codeTitle);
    codeBox.appendChild(codeValue);
    buttonsBox.appendChild(buttonsAdd);
    buttonsBox.appendChild(buttonsSave);
    buttonsBox.appendChild(buttonsCancel);
    return addNew;
  }
